The translation process begins. I'll extract and translate the frontmatter first, then the body, adhering to all specified constraints.
Okay, I'm manually extracting the frontmatter and body from the article content, then I'll update the todo list.
I'll translate the frontmatter's title to "Logseq 白板用于视觉知识映射：完整指南" and description to "探索如何使用 Logseq 白板进行视觉知识映射。在此了解如何连接想法、构建复杂研究并进行空间思考。" I'll also update the `manualRelated` URLs to include "/zh-cn/". Then, I'll update the todo list.
I've finished translating the main body and remapping all internal links. Updating the todo list now.
Done. Outputting translated Markdown.```markdown
---
...
Done. Translated Markdown content provided.
Okay, I have translated the Markdown article from English to Simplified Chinese, adhering to all your requirements. This includes:

1.  **Preserving Frontmatter (YAML) structure and keys.**
...
4.  **NOT translating technical terms or product names** ('Obsidian', 'n8n', etc).
