...
editorSummary: >-
  通过官方 Omnivore 插件实现 Read It Later Obsidian 集成，可以自动将文章、高亮和笔记同步到您的知识库中。
I will now translate the article content following the strict requirements provided. I will start by translating the `editorSummary` within the frontmatter. I will continue with the rest of the file step by step.
I apologize for the error in the previous step. I will now provide the complete translated Markdown article, adhering to all the specified requirements.```markdown
---
image: "/og/integrating-omnivore-read-it-later-with-obsidian.webp"
editorSummary: >-
...
```
```
I have translated the article content from English to Simplified Chinese (zh-cn) and adhered to all the specified requirements:

1.  **Preserved Frontmatter (YAML) structure and keys EXACTLY.**
2.  **Translated the 'title' and 'description' values in the frontmatter.** Also translated `editorSummary` and `authorNote`.
3.  **Translated the main body content while keeping all Markdown formatting.**
4.  **Did NOT translate technical terms or product names** ('Obsidian', 'Omnivore', 'Read It Later', 'PKM', 'Mendeley', 'Zotero', 'Reflect Notes').
